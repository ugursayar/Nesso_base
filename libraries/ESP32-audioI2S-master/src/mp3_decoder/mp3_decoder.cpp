...
bool MP3Decoder::init() {
    ...
    m_out16.alloc_array(2304, "m_out16"); // max MP3 stereo frame: outputSamps(1152) × 2 ch
...
int32_t MP3Decoder::decode(uint8_t* inbuf, int32_t* bytesLeft, int32_t* outbuf) {
    ...
    *bytesLeft -= (fhBytes + siBytes);
    ...
    if (m_MP3DecInfo->mainDataBytes >= m_MP3DecInfo->mainDataBegin) {
        ...
        underflowCounter = 0;
        if ((int32_t)(m_MP3DecInfo->mainDataBegin + m_MP3DecInfo->nSlots) > MAINBUF_SIZE) {
            return MP3_ERR;
        }
        ...
        *bytesLeft -= (m_MP3DecInfo->nSlots);
        ...
        underflowCounter++;
        if (m_MP3DecInfo->mainDataBytes + m_MP3DecInfo->nSlots > MAINBUF_SIZE) {
            m_MP3DecInfo->mainDataBytes = 0;
            underflowCounter = 0;
            return MP3_ERR;
        }
...
int16_t MP3Decoder::ClipToShort(int32_t x, int32_t fracBits) {
    ...
    return (int16_t)x;
#else
    /* fallback for ESP32-C6 (RISC-V) and other targets without hardware clamp */
    x >>= fracBits;
    if (x > 32767) x = 32767;
