...
constexpr size_t m_outbuffSize = 2304;           // reduced for no-PSRAM (ESP32-C6): was 4608*2
constexpr size_t m_samplesBuff48KSize = 2560;              // must fit resample output: 2304 × (48/44.1) ≈ 2508; 2560 gives safe margin

constexpr size_t AUDIO_STACK_SIZE = 8192; // increased for ESP32-C6: MP3 decode needs ~6-8KB stack; was 3500
...
AudioBuffer::AudioBuffer() {
    if(psramInit()) {
        m_mainBuffSize = UINT16_MAX * 10;  // ~640 KB with PSRAM
        m_resBuffSize  = UINT16_MAX;       // ~64  KB
    } else {
        m_mainBuffSize = 1024 * 24;        // 24 KB without PSRAM (e.g. ESP32-C6); total alloc = 28 KB
        m_resBuffSize  = 1024 * 4;         // 4  KB
    }
}
...
esp_err_t Audio::I2Sstart() {
    ...
    if (!m_f_i2s_channel_enabled) {
        ...
        else { AUDIO_LOG_ERROR("i2s_channel_enable failed: %d (0x%x)", err, err); }
    }
    return err;
}
// —————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————
...
void Audio::processWebStream() {
    ...
    // Wait for at least half the input buffer before starting playback; this gives ~0.75 s of pre-roll at 128 kbps
    // and prevents "slow stream" from firing immediately after connect on bursty CDN connections.
    if (!m_f_stream && InBuff.bufferFilled() > (InBuff.getBufsize() / 2)) {
        info(*this, evt_info, "stream ready");
...
bool Audio::setPinout(uint8_t BCLK, uint8_t LRC, uint8_t DOUT, int8_t MCLK) {
    ...
    if (!m_f_psramFound) {
        AUDIO_LOG_ERROR("PSRAM not found — running with reduced heap buffers");
        // No PSRAM on this board (e.g. ESP32-C6). Buffers are sized for heap-only
        // operation; continue without aborting.
    }
...
bool Audio::readMetadata(uint16_t maxBytes, uint16_t* readedBytes, bool first) {
    *readedBytes = 0;
    // ICY metadata in practice is well under 512 bytes; allocating 4096 would OOM on a fragmented ESP32-C6 heap
    const uint16_t META_BUFSIZE = 512;
    ps_ptr<char> buff;
    if (!buff.alloc(META_BUFSIZE + 1)) {
        AUDIO_LOG_WARN("readMetadata: OOM (%u bytes), skipping metadata block", META_BUFSIZE + 1);
        m_rmet.metaDataSize = 0;
        m_rmet.pos_ml = 0;
        return true; // don't crash; lose track title but keep playing
    }
    buff.clear();
    // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    if (first) {
    ...
    if (!m_rmet.metaDataSize) {
        ...
        m_rmet.metaDataSize = b * 16; // New count for metadata including length byte, max 4080
        m_rmet.pos_ml = 0;
    ...
    uint16_t toRead = min((uint16_t)(m_rmet.metaDataSize - m_rmet.pos_ml), maxBytes);
    if (m_rmet.pos_ml < META_BUFSIZE) {
        // Write into buffer while space remains
        uint16_t canWrite = min(toRead, (uint16_t)(META_BUFSIZE - m_rmet.pos_ml));
        int32_t a = audioFileRead((uint8_t*)&buff[m_rmet.pos_ml], canWrite);
        if (a > 0) {
            m_rmet.res += a;
            *readedBytes += a;
            m_rmet.pos_ml += a;
    ...
    // Drain overflow bytes (metaDataSize > META_BUFSIZE) in small chunks to maintain stream sync
    if (toRead > 0) {
        uint8_t sink[16];
        uint16_t drainChunk = min(toRead, (uint16_t)sizeof(sink));
        int32_t a = audioFileRead(sink, drainChunk);
        if (a > 0) {
            m_rmet.res += a;
            *readedBytes += a;
            m_rmet.pos_ml += a;
    ...
    if (m_rmet.pos_ml == m_rmet.metaDataSize) {
        uint16_t termPos = min((uint16_t)m_rmet.pos_ml, META_BUFSIZE);
        buff[termPos] = '\0';
...
int32_t Audio::getChunkSize(uint16_t* readedBytes, bool first) {
    ...
    *readedBytes = 0;
    ...
    while (true) {
        ...
        if (!m_client->available()) { vTaskDelay(1); continue; }
        int b = audioFileRead();
        if (b < 0) continue;
...
boolean Audio::streamDetection(uint32_t bytesAvail) {
    ...
    if (m_sdet.tmr_lost < millis()) {
        m_sdet.cnt_lost++;
        m_sdet.tmr_lost = millis() + 1000;
        if (m_sdet.cnt_lost == 8) { // 8s no data?
            m_sdet.cnt_lost = 0;
...
void Audio::performAudioTask() {
    ...
    while (m_validSamples) {
        ...
        int32_t prevSamples = m_validSamples;
        playChunk();
        if (m_validSamples == prevSamples && ++i2s_retry > 10) {
            AUDIO_LOG_ERROR("I2S write stalled, discarding %d samples", m_validSamples);
            m_validSamples = 0;
            break;
        }
    } // I2S buffer full
